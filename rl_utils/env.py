...
class UpperEnv:
    ...
    def step(self, action:int):
        ...
        while not lower_done:
            ...
            if self.lower_type == 'DQN':
                ...
                lower_action = int(self.lower_agent(torch.tensor(lower_state, dtype=torch.float32).unsqueeze(0)).argmax())
        ...
        match_rate = self.is_match_compute_tuple[1]/(self.is_match_compute_tuple[0]+0.1) if (lower_step_cnt<=lower_env.max_step) else 0
        reward *= (1 - match_rate)
    ...
    def step_with20action(self, action:int):
       ...
       # state computing with neighbor rts
        ...
        # using v_avg as state, v_avg_upper = distance/delta_t
        self.v_avg = 0
        v_upper = (abs(goal_pos[0] - start_pos[0])**2 + abs(goal_pos[1] - start_pos[1])**2)**0.5 /(t_upper+1) * 60
        self.v_avg += (v_upper - self.v_avg)/(self.step_cnt+1)
        ...
        reward -= abs(z_score)*(1- self.is_match_compute_tuple[1]/(self.is_match_compute_tuple[0]+0.1)) # *factor_dict[upper_mode]
    ...
    def reset(self):
        ...
        return np.array( [relative_dis/(t+0.1)] + [0,0,0,0] + self.rts_nums )
